...
namespace RetroGamingTournament.Repositories
{
    public class TournamentRepository : ITournamentRepository
    {
        ...
        {
            return await _context.Tournaments.Where(x => x.Id == id).Include(x => x.Game).Include(x => x.Event).Include(x => x.Stages).Include(x => x.Players).Include(x => x.Groups).FirstAsync();
        }
